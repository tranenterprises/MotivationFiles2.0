...
async function testDatabaseConnection() {
  ...
  try {
    ...
    console.log(
      `✅ Connection successful! Found ${count} quotes in database\n`
    );
    ...
    if (categoryError) {
      console.error(
        '❌ Failed to get motivation quotes:',
    ...
    if (todayQuote) {
      console.log(
        `✅ Found quote for today: ${todayQuote.content.substring(0, 50)}...`
      );
    } else {
      console.log(
        "📝 No quote found for today (this is expected if not seeded with today's date)"
      );
    }
    console.log();
