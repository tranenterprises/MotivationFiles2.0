...
if (!projectRef || !cronSecret) {
  console.error('Usage: node scripts/test-deployment.js <project-ref> <cron-secret>');
  console.error('Example: node scripts/test-deployment.js abcdefghijklmnop your-secret-key');
  process.exit(1);
}
...
async function testHealthCheck() {
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      console.log(`✅ Environment: ${response.data.checks?.environment ? 'PASS' : 'FAIL'}`);
      console.log(`✅ Database: ${response.data.checks?.database ? 'PASS' : 'FAIL'}`);
      console.log(`✅ OpenAI: ${response.data.checks?.openai ? 'PASS' : 'FAIL'}`);
      console.log(`✅ ElevenLabs: ${response.data.checks?.elevenlabs ? 'PASS' : 'FAIL'}`);
...
async function testQuoteGeneration() {
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      console.log(`✅ Quote: ${response.data.quote?.content?.substring(0, 100)}...`);
...
async function runTests() {
  ...
  console.log(`Quote Generation: ${results.generation ? '✅ PASS' : '❌ FAIL'}`);
