  ...
  try {
    ...
    if (!checkError) {
      ...
      if (count > 0) {
        ...
        samples?.forEach(quote => {
          console.log(
            `- ${quote.date}: ${quote.category} - ${quote.content.substring(0, 50)}...`
          );
        });
        console.log();
      }
      ...
      console.log(
        '📝 Quotes table does not exist. This is expected for a new project.'
      );
      console.log('❌ Error details:', checkError.message);
      console.log(
        '🔧 You may need to run the migration manually via Supabase dashboard or CLI.\n'
      );
    }
    ...
    if (!tablesError && tables) {
      ...
      if (!authError) {
        ...
        console.log(
          '⚠️  Connection test results unclear, but service is likely working'
        );
      }
    }
    ...
    console.log(
      '2. If quotes table is missing: Run the migration via Supabase dashboard:'
    );
    console.log(
      '   - Go to: https://supabase.com/dashboard/project/jrbkzqbutldvkfoaxgry'
    );
