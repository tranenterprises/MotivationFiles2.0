...
test.describe('Homepage', () => {
  ...
  test('should display hero section with motivational content', async ({ page }) => {
    // Check hero content
    await expect(page.getByText('FUEL YOUR')).toBeVisible()
    await expect(page.getByText('GRIND')).toBeVisible()
    await expect(page.getByText('Daily motivation delivered with the intensity')).toBeVisible()
  ...
  test('should handle no quote available gracefully', async ({ page }) => {
    ...
    if (hasNoQuote) {
      await expect(page.getByText('Check back later or visit the archive')).toBeVisible()
    }
  })

  test('should display quote with proper styling when available', async ({ page }) => {
    const quoteContent = page.locator('blockquote')
  ...
  test('should display footer', async ({ page }) => {
    await expect(page.getByText('© 2024 Daily Motivation Voice App')).toBeVisible()
