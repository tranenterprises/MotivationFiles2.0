...
test.describe('Archive Page', () => {
  ...
  test('should display statistics', async ({ page }) => {
    ...
    const statsNumbers = page.locator('.text-accent').filter({ hasText: /^\d+$/ })
  ...
  test('should display quotes in grid layout when available', async ({ page }) => {
  ...
  test('should show audio buttons for quotes with audio', async ({ page }) => {
    ...
    if (audioButtonCount > 0) {
      ...
      await expect(firstAudioButton).toHaveAttribute('aria-label', 'Play audio')
    }
  })
