...
function parseAudioFormat(formatString: string): {
  format: string;
  sampleRate: number;
  bitrate?: number;
} {
...
async function generateVoiceWithRetry(
  ...
  try {
    ...
    console.error(
      `Voice generation attempt ${attempt} with voice ${currentVoiceId} failed:`,
      error.message
    );
    ...
    if (isVoiceError(error) && currentVoiceId === FALLBACK_VOICE_ID) {
      ...
      return generateVoiceWithRetry(
        text,
        options,
        attempt,
        EMERGENCY_FALLBACK_VOICE_ID
      );
    }
    ...
    throw new Error(
      `Failed to generate voice after ${attempt} attempts: ${error.message}`
    );
  }
}
...
export async function generateVoiceHighQuality(
  text: string
): Promise<VoiceGenerationResult> {
  return generateVoice(text, { format: 'mp3', quality: 'high' });
}

export async function generateVoiceStandard(
  text: string
): Promise<VoiceGenerationResult> {
  return generateVoice(text, { format: 'mp3', quality: 'standard' });
}

export async function generateVoiceCompressed(
  text: string
): Promise<VoiceGenerationResult> {
...
export async function generateVoiceAndUpload(
  ...
  const uploadResult = await uploadQuoteAudio(
    quoteId,
...
export async function generateVoiceAndUploadHighQuality(
  ...
  return generateVoiceAndUpload(text, quoteId, {
    format: 'mp3',
    quality: 'high',
  });
}
...
export async function generateVoiceWithFallbacks(
  ...
  for (let i = 0; i < fallbackStrategies.length; i++) {
    try {
      console.log(
        `Attempting voice generation strategy ${i + 1}/${fallbackStrategies.length}`
      );
      ...
      if (i === fallbackStrategies.length - 1) {
        throw new Error(
          `All voice generation strategies failed. Last error: ${error.message}`
        );
      }
...
export async function generateVoiceWithFallbacksAndUpload(
  ...
  try {
    ...
    console.error(
      'Upload failed after successful voice generation:',
      uploadError.message
    );
    throw new Error(
      `Voice generation succeeded but upload failed: ${uploadError.message}`
    );
  }
}
