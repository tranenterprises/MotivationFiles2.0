...
describe('Cache Utilities', () => {
  ...
  describe('Cache Key Generation', () => {
    it('should generate archive cache keys correctly', () => {
      ...
      expect(generateArchiveCacheKey(2, 'motivation')).toBe(
        'cache:archive_p2_l12_cat-motivation'
      );
      expect(generateArchiveCacheKey(3, undefined, 20)).toBe(
        'cache:archive_p3_l20'
      );
    });
  ...
  describe('withCache Wrapper', () => {
    it('should return cached data if available', async () => {
      ...
      const result = await withCache('test-key', apiCall, {
        storage: 'memory',
      });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        withCache('test-key', apiCall, { storage: 'memory' })
  ...
  describe('Cache Management', () => {
    ...
    describe('invalidateCache', () => {
      it('should invalidate cache entries matching patterns', () => {
        setCachedData('cache:today_quote_2024-01-15', mockQuote, {
          storage: 'memory',
        });
        setCachedData('cache:archive_p1_l12', [mockQuote], {
          storage: 'memory',
        });
        setCachedData(
          'cache:other_data',
        ...
        expect(
          getCachedData('cache:today_quote_2024-01-15', { storage: 'memory' })
        ).toBeNull();
        expect(
          getCachedData('cache:archive_p1_l12', { storage: 'memory' })
        ).toBeNull();
        expect(
          getCachedData('cache:other_data', { storage: 'memory' })
        ).toEqual({ other: true });
      });

      it('should handle localStorage invalidation', () => {
        localStorage.setItem(
          'cache:today_quote_2024-01-15',
          JSON.stringify({
            data: mockQuote,
            timestamp: Date.now(),
            ttl: 300000,
  ...
  describe('preloadCache', () => {
    it('should preload essential cache data', async () => {
      const { getTodaysQuote, getAllQuotes, getQuoteCount } = await import(
        './supabase'
      );
    ...
    it('should handle preload errors gracefully', async () => {
      const { getTodaysQuote } = await import('./supabase');
      (getTodaysQuote as jest.Mock).mockRejectedValue(
