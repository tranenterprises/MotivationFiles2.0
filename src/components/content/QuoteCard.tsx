...
export default function QuoteCard({
  ...
  return (
    <div className="fade-in">
      <div
        className={`bg-gray-800 border border-gray-600 rounded-lg ${classes.container} hover:border-accent transition-colors duration-300 group`}
      >
        <div className="mb-6">
          <p
            className={`uppercase tracking-wide text-accent font-semibold mb-2 ${classes.category}`}
          >
            {quote.category}
          </p>
        ...
        <blockquote
          className={`quote-text text-white leading-tight mb-8 ${classes.quote}`}
        >
          "{quote.content}"
        </blockquote>
          ...
          {showAudio && quote.audio_url && (
            ...
            >
              {isAudioLoading ? (
                ...
                <svg
                  className={`text-white fill-current ${classes.audioButton}`}
                  viewBox="0 0 24 24"
                >
