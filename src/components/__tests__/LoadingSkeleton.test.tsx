...
describe('LoadingSkeleton Component', () => {
  ...
  describe('Quote Card Variant', () => {
    ...
    it('renders quote card skeleton with small size', () => {
      const { container } = render(
        <LoadingSkeleton variant="quote-card" size="small" />
      );
    ...
    it('includes all quote card skeleton elements', () => {
      ...
      // Quote text lines
      const quoteLines = container.querySelectorAll(
        '.space-y-3 .h-6.bg-gray-700.rounded'
      );
      expect(quoteLines.length).toBeGreaterThan(0);

      // Audio button placeholder
      const audioButton = container.querySelector(
        '.w-10.h-10.bg-gray-700.rounded-full'
      );
    ...
    it('renders multiple quote card skeletons', () => {
      const { container } = render(
        <LoadingSkeleton variant="quote-card" count={3} />
      );
  ...
  describe('Quote Grid Variant', () => {
    ...
    it('ignores count parameter for quote-grid (always shows 6)', () => {
      const { container } = render(
        <LoadingSkeleton variant="quote-grid" count={3} />
      );
  ...
  describe('Animation and Styling', () => {
    ...
    it('uses consistent gray color scheme', () => {
      ...
      const grayElements = container.querySelectorAll(
  ...
  describe('Edge Cases', () => {
    it('handles zero count gracefully', () => {
      const { container } = render(
        <LoadingSkeleton variant="text" count={0} />
      );
