...
function QuoteCardSkeleton({ size = 'medium' }: { size: 'small' | 'medium' | 'large' }) {
  const sizeClasses = {
    small: 'p-4',
    medium: 'p-6', 
    large: 'p-8'
  ...
  return (
    <div className="fade-in">
      <div className={`bg-gray-800 border border-gray-600 rounded-lg ${sizeClasses[size]} animate-pulse`}>
        {/* Category and Date */}
        <div className="mb-6">
...
function TextSkeleton({ width = 'full' }: { width?: string }) {
  return <div className={`h-4 bg-gray-700 rounded animate-pulse w-${width}`}></div>
}
