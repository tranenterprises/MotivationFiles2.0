  ...
  return function MockHeroFallback() {
    return (
      <div data-testid="hero-fallback">
        ...
        <button data-testid="fallback-check-again" onClick={() => window.location.reload()}>
          Check Again
        </button>
      </div>
    )
...
describe('HeroSection', () => {
  ...
  describe('with valid quote', () => {
    ...
    it('displays date at the top of the quote section', () => {
      ...
      expect(dateElement).toHaveClass('text-accent', 'font-bold', 'uppercase', 'tracking-widest')
    })
  ...
  describe('with error state', () => {
    it('renders error fallback when hasError is true', () => {
      ...
      expect(screen.getByText('Fallback Content: loading-error')).toBeInTheDocument()
    })
  ...
  describe('responsive design classes', () => {
    it('applies mobile-first responsive classes', () => {
      ...
      const categorySection = screen.getAllByText('motivation')[0].closest('div')?.parentElement
