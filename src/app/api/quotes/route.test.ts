...
>;
const mockGetAllQuotes = getAllQuotes as jest.MockedFunction<
  typeof getAllQuotes
>;
const mockGetQuotesByCategory = getQuotesByCategory as jest.MockedFunction<
  typeof getQuotesByCategory
>;
const mockGetQuoteCount = getQuoteCount as jest.MockedFunction<
  typeof getQuoteCount
>;
const mockGetQuotesByDateRange = getQuotesByDateRange as jest.MockedFunction<
  typeof getQuotesByDateRange
>;
...
const mockQuotes = [
  mockQuote,
  { ...mockQuote, id: '2', content: 'Second quote' },
];
...
describe('Quotes API Route', () => {
  ...
  describe("Today's Quote (type=today)", () => {
    it("should return today's quote when available", async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=today'
      );
    ...
    it('should handle database errors gracefully', async () => {
      mockGetTodaysQuote.mockRejectedValue(
        new Error('Failed to fetch quote from database')
      );
  ...
  describe('Archive (type=archive)', () => {
    it('should return paginated archive quotes', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=archive&limit=20&offset=0'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should use default pagination values', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=archive'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should reject limit over 100', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=archive&limit=150'
      );
      const response = await GET(request);
      const data = await response.json();
  ...
  describe('Category Filtering (type=category)', () => {
    it('should return quotes for valid category', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=category&category=motivation'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should reject missing category parameter', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=category'
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(data.error).toBe(
        'Category parameter is required for category type'
      );
    ...
    it('should reject invalid category', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=category&category=invalid'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should accept all valid categories', async () => {
      ...
      for (const category of validCategories) {
        const request = new NextRequest(
          `http://localhost:3000/api/quotes?type=category&category=${category}`
        );
        const response = await GET(request);
        const data = await response.json();
  ...
  describe('Date Range (type=date_range)', () => {
    it('should return quotes for valid date range', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&start_date=2024-01-01&end_date=2024-01-31'
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(mockGetQuotesByDateRange).toHaveBeenCalledWith(
        '2024-01-01',
        '2024-01-31'
      );
    });
    ...
    it('should reject missing start_date parameter', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&end_date=2024-01-31'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should reject missing end_date parameter', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&start_date=2024-01-01'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should reject invalid date format', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&start_date=01-01-2024&end_date=2024-01-31'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should reject invalid date range (start after end)', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&start_date=2024-01-31&end_date=2024-01-01'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should accept equal start and end dates', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=date_range&start_date=2024-01-01&end_date=2024-01-01'
      );
      const response = await GET(request);
      const data = await response.json();
  ...
  describe('Invalid Type Parameter', () => {
    it('should reject invalid type parameter', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/quotes?type=invalid'
      );
      const response = await GET(request);
      const data = await response.json();
